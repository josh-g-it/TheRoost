...
### AI Assistant
A conversational AI assistant that knows your gaming library and can help you manage it:
- **Chat interface**: Ask questions about your games, get recommendations, or just chat about gaming. Multi-turn conversations with full history.
- **Smart actions**: Say things like "filter to RPGs", "sort by playtime", or "show me my favorites" — the assistant understands and takes action instantly. Some actions (like adding a note or rating a game) ask for confirmation first.
...
- **AI Assistant**: Chat with the assistant while gaming — same conversation synced with the main window
...
### Settings
Organized into 6 tabs for quick access: General, Assistant, Connections, Appearance, Navigation, and Advanced.
...
│   │   ├── assistant/      ← AI assistant (chat, memories, journals, avatars)
│   │   ├── overlay/        ← System overlay panels (6 HUD panels)
...
└── src-tauri/              ← The engine (behind the scenes)
    └── src/
        ├── commands/       ← 169 things the frontend can ask the backend to do
        ├── models/         ← Data shapes (60+ structs)
        ├── services/       ← Core logic (45 modules: APIs, database, monitoring,
        │                      audio, media, overlay, AI assistant, launcher scanners, recaps)
...
| **SQLite** for local data | Fast, reliable local database (29 tables) — no server needed |
...
| **Gemini** | Google's AI model family. The Roost uses Gemini 3 Flash for conversational AI responses. |
| **Pattern matcher** | A fast, offline system that recognizes common requests (like "filter to RPGs") without calling the cloud AI. |
| **Memory vault** | A collection of facts the AI assistant remembers about you across conversations (e.g., your favorite genres). |
| **Daily journal** | An auto-generated narrative summary of your gaming sessions for a given day. |
| **Avatar** | A named AI persona with its own personality and conversation style. |
| **Action chip** | A clickable button in the AI chat that performs a game action (like rating a game or adding a note). |
