...
│   │   │                         #   OverlayAudioMixer, OverlayAssistant, OverlayTagFilter,
│   │   │                         #   Sparkline, utils/ (overlayPanelRegistry, panelCollision)
│   │   ├── assistant/            # AssistantView (chat UI), AssistantChat,
│   │   │                         #   AssistantInput, AssistantMessage, AssistantToolbar,
│   │   │                         #   AssistantMemories, AssistantJournals,
│   │   │                         #   AssistantAvatars, AssistantFirstRun
...
│   │   └── useAssistantChat.ts   # AI conversation lifecycle (send, stream, actions)
│   │
...
│       ├── lib.rs                # Tauri setup, tracing init, 169 commands, background services
│       │
│       ├── commands/             # 32 command modules, 169 total commands (see §3.1)
...
│       │   └── mod.rs
│       │
│       ├── models/               # 22 model files, 60+ structs (see §3.3)
...
│       │   └── mod.rs
│       │
│       ├── services/             # 45 service modules (see §3.5)
│       │   ├── cache_db.rs       # SQLite: schema v26, 29 tables, WAL mode
...
│       │   │        cloud_config, cloud_cache, conversation, conversation_timer,
│       │   │        memory, encryption, providers/gemini)
│       │   └── mod.rs
│       │
│       └── utils/
│           ├── mod.rs
...
169 Tauri commands across 32 modules:
...
| `overlay` | `toggle_overlay`, `hide_overlay`, `show_main_and_navigate`, `overlay_select_game`, `update_overlay_shortcut`, `get_overlay_library`, `overlay_apply_tag_filter`, `notify_settings_changed`, `overlay_execute_palette_action`, `overlay_relay_ai_event` |
...
| `ai` | `ai_resolve_intent`, `ai_cloud_resolve`, `ai_stream_cloud_resolve`, `store_cloud_api_key`, `delete_cloud_api_key`, `get_cloud_api_key_status`, `test_cloud_api_key`, `get_cloud_ai_usage`, `update_cloud_ai_settings`, `ai_get_conversations`, `ai_get_conversation`, `ai_create_conversation`, `ai_delete_conversation`, `ai_send_message`, `ai_get_memories`, `ai_create_memory`, `ai_update_memory`, `ai_delete_memory`, `ai_toggle_memory`, `ai_get_daily_logs`, `ai_get_daily_log`, `ai_generate_daily_log`, `ai_delete_daily_log`, `ai_get_avatars`, `ai_create_avatar`, `ai_update_avatar`, `ai_delete_avatar`, `ai_get_personalities`, `ai_create_personality`, `ai_update_personality`, `ai_delete_personality`, `ai_get_active_persona`, `ai_set_active_persona`, `ai_end_conversation`, `ai_get_system_prompt_preview` |
...
- Accounts: `steam_api_key`, `sgdb_api_key`, `cloud_ai_gemini` (Gemini API key), `ai_encryption_key` (conversation data encryption)
...
**Current schema version: v26** — Location: `%APPDATA%/app.theroost/theroost.db`
...
| `ai_conversations` | Conversation threads (title, avatar, personality, timestamps) | `id TEXT` |
| `ai_messages` | Chat messages (role, content, actions, encrypted) | `id TEXT` |
| `ai_memories` | User/AI-created memories (content, source, active/inactive) | `id TEXT` |
| `ai_daily_logs` | Auto-generated daily session summaries (encrypted) | `id TEXT` |
| `ai_avatars` | Custom AI personas (name, description, system prompt) | `id TEXT` |
| `ai_personalities` | Conversation style presets (tone, verbosity, emoji usage) | `id TEXT` |

Database features: WAL mode, foreign keys enforced (ON DELETE CASCADE on 13 game child tables), 7+ indexes for query performance. v26 added UNIQUE indexes on `ai_avatars.name` and `ai_personalities.name`.

Migration system checks `user_version` pragma and applies incremental migrations v1→v26.
...
### 3.16 AI Assistant System (v1.12.0)

The Conversational AI Assistant is a two-tier architecture that handles both instant game actions and multi-turn conversational AI.

#### Two-Tier Resolution

```
User input → ai_resolve_intent() [Rust pattern matcher]
  → Match found (confidence ≥ threshold):
      Return AiResponse with action(s) + natural-language reply
  → No match:
      Forward to Gemini 3 Flash cloud AI via ai_cloud_resolve() / ai_stream_cloud_resolve()
      → 4-layer context assembly (see below)
      → Streaming token response via Tauri events
      → Parse structured actions from AI response
```

**Pattern matcher** (`services/ai/pattern_matcher.rs`):
- 10 intent extractors: launch, playtime, achievements, recommend, filter, sort, search, navigate, favorite, rate/review/note/hide
- Game targeting via fuzzy name matching against the full library
- Returns structured `AiAction` payloads with confidence scores

**Cloud AI** (`services/ai/providers/gemini.rs`):
- Provider: Gemini 3 Flash (`gemini-3-flash-preview`)
- Streaming responses via SSE (Server-Sent Events)
- Structured action extraction from markdown-formatted responses
- Rate limiting: configurable daily message cap (default 50)

#### 4-Layer Context System (`services/ai/context_builder.rs`)

```
Layer 1: System prompt — persona instructions, action schema, library summary
Layer 2: Library context — game list with playtime, favorites, ratings, tags (scope: all/favorites/custom)
Layer 3: Memory vault — active user/AI memories injected as context
Layer 4: Conversation history — recent messages from the active conversation thread
```

Context scope is user-configurable (all games, favorites only, or custom include/exclude lists). Total context budget targets ~8K tokens per request.

#### AI Actions

Actions are categorized into two tiers:

| Tier | Behavior | Actions |
|------|----------|---------|
| T1 (auto-execute) | Executed immediately without confirmation | `filter`, `sort`, `search`, `navigate`, `clear_filters` |
| T2 (confirm) | Shown as action chips requiring user click | `favorite`, `unfavorite`, `rate`, `review`, `note`, `hide`, `unhide` |

Actions include a `gameId` (resolved via fuzzy matching) and are rendered as interactive chips in the chat UI.

#### Conversation & Message Persistence

- `ai_conversations` table: thread metadata (title, avatar, personality, created/updated timestamps)
- `ai_messages` table: individual messages with `role` (user/assistant/system), content, optional action payload
- Conversations support rename, delete, and auto-titling (first user message becomes the title)
- Cross-window sync: conversation events relayed between main and overlay via `overlay_relay_ai_event`

#### Memory System

- **AI memories** (`ai_memories` table): key insights extracted from conversations, stored with source attribution
- Auto-created by the AI during conversations (e.g., "User prefers RPGs", "User's favorite game is Skyrim")
- Users can manually create, edit, toggle (active/inactive), or delete memories
- Active memories are injected into the context builder for every AI request
- System memories (auto-created) hidden from the UI by default; user memories always visible

#### Daily Journals (`ai_daily_logs` table)

- Auto-generated summaries of the user's gaming sessions for a given day
- Created via `ai_generate_daily_log` which aggregates session data + AI-generated narrative
- Encrypted at rest using the app's encryption key
- Browsable in the Assistant Journals sub-page

#### Avatar & Personality System

- **Avatars** (`ai_avatars` table): named AI personas with custom system prompt fragments
- **Personalities** (`ai_personalities` table): conversation style presets (tone, verbosity, emoji usage)
- Active persona = avatar + personality pair, persisted per-conversation and as a global default
- UNIQUE indexes on `ai_avatars.name` and `ai_personalities.name` (schema v26)

#### Encryption (`services/ai/encryption.rs`)

- Conversation content and daily logs encrypted at rest using AES-256-GCM
- Encryption key stored in Windows Credential Manager (`ai_encryption_key` account)
- Key auto-generated on first use; transparent encrypt/decrypt in the service layer

#### Cloud AI Settings (three-layer enforcement)

```
Layer 1: API key presence — no key = cloud AI fully disabled
Layer 2: Global toggle — user can disable cloud AI even with a valid key
Layer 3: Per-request check — daily limit enforcement + active conversation validation
```

Settings stored in `settings.json`: `cloud_ai_provider`, `cloud_ai_daily_limit`, `cloud_ai_context_scope`, `cloud_ai_exclude_games`, `cloud_ai_include_games`, `cloud_ai_post_session_review`.

#### Frontend Components (`src/components/assistant/`)

| Component | Purpose |
|-----------|---------|
| `AssistantView` | Page shell with sub-page navigation (chat, memories, journals, avatars) |
| `AssistantChat` | Conversation list + message thread with streaming indicator |
| `AssistantInput` | Message input with send button, character limit, streaming awareness |
| `AssistantMessage` | Individual message bubble (user/assistant), action chip rendering |
| `AssistantToolbar` | Conversation management (new, rename, delete, persona picker) |
| `AssistantMemories` | Memory vault browser (create, edit, toggle, delete) |
| `AssistantJournals` | Daily journal browser with date navigation |
| `AssistantAvatars` | Avatar & personality CRUD with preview |
| `AssistantFirstRun` | First-run wizard for AI setup (API key, persona selection) |

#### Frontend Hook (`src/hooks/useAssistantChat.ts`)

Manages the full conversation lifecycle:
- Send message → optimistic UI update → invoke backend → stream tokens → finalize
- Action chip click → execute T2 action → update chat with result
- Conversation switching, creation, deletion
- Streaming state management (loading indicator, partial message assembly)

#### Overlay Integration

- 6th FloatingPanel: `OverlayAssistant` — lightweight chat interface in the overlay
- Shares conversation state with main window via `overlay_relay_ai_event` Tauri event
- Streaming responses work in both main window and overlay simultaneously
...
main.tsx → ErrorBoundary → App
  ...
  Routes (all content routes have errorElement: <RouteErrorFallback />):
    /          → redirect to /library
    /library   → LibraryView
    /assistant → AssistantView (chat, memories, journals, avatars sub-pages)
...
| `aiSlice` | `conversations`, `activeConversationId`, `messages`, `memories`, `dailyLogs`, `avatars`, `personalities`, `isStreaming` | AI assistant state |
...
- 6 panels: Command Center, Game Notes, System Monitor, Media Controls, Audio Mixer, Assistant
...
| `settings.ts` | `AppSettings`, `OverlayPanelId` (6 panels), `OverlayPanelPosition` |
...
| `ai.ts` | `AiConversation`, `AiMessage`, `AiMemory`, `AiDailyLog`, `AiAvatar`, `AiPersonality`, `AiAction`, `AiActionTier`, `AiResponse` |

### 4.6 Frontend API Layer (services/tauri.ts)

30 API namespaces wrapping `invoke()` calls (selected highlights):
...
| `aiApi` | 35 | AI assistant: conversations, messages, memories, journals, avatars, personas, streaming |
| `updaterApi` | 3 | Update check, install, version |
| `autostartApi` | 2 | Launch-on-startup toggle |
| `cloudAiApi` | 8 | Cloud AI key management, resolve, usage, settings (Gemini 3 Flash) |
...
OverlayApp:
  ...
  → Renders OverlayBackdrop + OverlayWindowManager + 6 FloatingPanels
...
cloud_ai_post_session_review, active_avatar_id, active_personality_id
```
...
**Shared test factories** (`src/test/factories.ts`): `makeGame()`, `makeMeta()`, `makeFilters()`, `makeSession()`, `makeShelf()`, `makeRecap()`, `makeInstallProgress()`, `ts()`, `makeAiAvatar()`, `makeAiMessage()`, `makeAiMemory()`, `makeAiPersonality()`, `makeAiDailyLog()`. Override object pattern for all factories. `makeGame` includes `description: null` by default. `makeShelf` includes `pinnedGameIds: []` by default.
...
| v1.12.0 | Done | Conversational AI Assistant — see §3.16 for full details |
